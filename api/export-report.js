...
export default async function handler(req, res) {
  ...
  try {
    ...
    accounts.forEach(acc => {
      ...
      (txByAccount[acc.id] || []).forEach(tx => {
        if (tx.accountId === acc.id) {
          ...
          if (tx.type === 'debt') currentBalance += (tx.category === 'Pelunasan' ? -tx.amount : (tx.isInitial ? 0 : tx.amount));
          if (tx.type === 'transfer') currentBalance -= (tx.category === 'Piutang' && !tx.isInitial ? tx.amount : tx.amount);
          if (tx.type === 'asset') currentBalance += (tx.category === 'Settlement' ? tx.amount : -tx.amount);
        }
        if (tx.targetAccountId === acc.id) currentBalance += tx.amount;
      });
    ...
    await transporter.sendMail({
      ...
      html: `<div style="font-family:Arial;padding:20px;border:1px solid #eee;border-radius:12px;"><h3>Laporan SteFin</h3>${aiAnalysis || 'Analisis tertunda.'}<br/><p>Total In: ${fmt(income)} | Out: ${fmt(expense)}</p></div>`,
