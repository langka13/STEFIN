            ...
            {[t('login'),t('register')].map((label, i) => (
              <button
                ...
                className={`rounded-xl py-2.5 text-sm font-medium transition ${
                  (i===1) === isRegister ? 'bg-gradient-to-r from-emerald-400 to-teal-500 shadow-lg shadow-emerald-500/20 text-white dark:text-black text-slate-950' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-slate-50'
                }`}
              >
                {label}
              </button>
    ...
    <section className="space-y-6">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-6">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className={`rounded-full px-3 py-1 text-xs font-outfit font-semibold border ${
              savingsRate >= 20 ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-500/20' :
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {assistantMsgs.map((m,i) => (
            <div key={i} className={`rounded-2xl border p-4 text-sm font-medium ${
              m.type==='danger'  ? 'border-rose-100 dark:border-rose-500/20  bg-rose-50 dark:bg-rose-500/10  text-rose-600 dark:text-rose-400'  :
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {filteredTx.map(tx => (
                  <tr key={tx.id} className="transition hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                    ...
                    <td className={`px-4 py-4 text-right font-outfit font-semibold text-sm ${
                      tx.type==='income'||tx.category==='Settlement'||(tx.type==='debt'&&tx.category!=='Pelunasan'&&tx.isInitial===false)
                        ?'text-emerald-400'
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
          <button
            ...
            className={`flex-1 py-1.5 text-sm font-medium rounded-lg transition-colors ${
              lang === 'id' ? 'bg-white dark:bg-slate-700 shadow-sm text-emerald-600 dark:text-emerald-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'
            }`}
          >
            {t('lang_id')}
          </button>
          <button
            ...
            className={`flex-1 py-1.5 text-sm font-medium rounded-lg transition-colors ${
              lang === 'en' ? 'bg-white dark:bg-slate-700 shadow-sm text-emerald-600 dark:text-emerald-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'
            }`}
          >
            {t('lang_en')}
          </button>
